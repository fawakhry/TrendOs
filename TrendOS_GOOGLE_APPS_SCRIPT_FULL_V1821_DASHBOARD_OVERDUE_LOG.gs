...
function isHiddenFromUserScreens_(status) {
  const s = normalize_(status);
  return s === "جاهز للاستلام" || s === "تم التسليم" || s === "مكرر";
}
...
function getDashboard_(e) {
  ...
  if (lines.getLastRow() < 2) return { success: true, dashboard: emptyDashboard_() };
  ...
  const dashboard = emptyDashboard_();
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (received) {
      ...
      r.setHours(0, 0, 0, 0);
      if (r.getTime() === today.getTime() && orderId) todayOrders[orderId] = true;
    }

    if (!isHiddenFromUserScreens_(status)) {
      if (orderId) activeOrders[orderId] = true;
      ...
      if (dept === "مكبس" || isHeatPressFlag_(valueAt_(row, colPress))) {
        dashboard.byDepartment["مكبس"] = (dashboard.byDepartment["مكبس"] || 0) + 1;
      }
      if (isOverdueByExpected_(status, expectedRaw)) dashboard.overdue++;
      if (status === "مشكلة" || status === "متوقف") dashboard.problems++;
    }

    if (status === "جاهز للاستلام") dashboard.readyForPickup++;
    if (status === "تم التسليم") dashboard.delivered++;
    if (status === "مكرر") dashboard.duplicate++;
  }

  dashboard.todayOrders = Object.keys(todayOrders).length;
  dashboard.activeOrders = Object.keys(activeOrders).length;
...
function resetTrendOSValidations_() {
  ...
  if (lines) {
    ...
    setDropdownByHeader_(lines, h, ["الحالة", "Status"], ["طلب جديد", "جاهز للطباعة", "بدأ التنفيذ", "تحت التنفيذ", "تم التنفيذ", "جاهز للاستلام", "تم التسليم", "مكرر", "مشكلة", "متوقف"]);
  ...
  if (orders) {
    ...
    setDropdownByHeader_(orders, h, ["الحالة العامة", "الحالة", "Status"], ["طلب جديد", "جاهز للطباعة", "بدأ التنفيذ", "تحت التنفيذ", "تم التنفيذ", "جاهز للاستلام", "تم التسليم", "مكرر", "مشكلة", "متوقف"]);
