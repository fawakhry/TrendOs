// TrendOS Tasks V3 T1.5 v5 — SERVICE-BINDING VERSION-OVERRIDE PREVIEW WORKER
// ISOLATED PREVIEW ONLY. DO NOT bind to trendos-main or production routes.
// Read path: Worker -> isolated D1 only.
// Async refresh: scheduled Worker -> T1 Service Binding pinned to qualified version -> Apps Script -> Sheets.
...
const TASKS_V3_T1_SERVICE_NAME = 'trendos-tasks-v3-t1-preview-20260914';
const TASKS_V3_T1_VERSION_ID = 'daf384a9-27ae-4260-bf53-1757555caca0';
    ...
    const request = new Request('https://tasks-v3-t1.internal/', {
      ...
      headers: {
        ...
        'Cloudflare-Workers-Version-Overrides':
          `${TASKS_V3_T1_SERVICE_NAME}="${TASKS_V3_T1_VERSION_ID}"`
      },
      body: JSON.stringify({
        op: text(op),
...
async function getStatusWithRetry({ env, operator, role, sourceImpl }) {
  ...
  if (result && result.success === true && validStatusBody(result.body)) return result;
...
async function refreshTasksV3T15Snapshot({
  ...
  `).bind(
    ...
    healthJson,
    statusJson,
    'TASKS_V3_T15_D1_PREVIEW_5_VERSION_OVERRIDE'
  ).run();
