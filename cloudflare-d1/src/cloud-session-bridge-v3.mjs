...
export async function verifyEmployeeSessionViaPost(username, employeeToken, env, lane = 'edge') {
  ...
  const payload = { action: 'verifyEmployeeSession', username: text(username), token: text(employeeToken), _ts: Date.now() };
  ...
  try {
    ...
    catch { return { ok: false, kind: 'upstream', status: response.status, message: 'Apps Script verification returned invalid JSON' }; }
    if (!response.ok) return { ok: false, kind: 'upstream', status: response.status, message: text(body && body.message) || `Apps Script verification failed (HTTP ${response.status})` };
    if (!body || body.success !== true) return { ok: false, kind: 'auth', status: response.status, message: text(body && body.message) || 'Employee session rejected' };
    return { ok: true, body };
  } finally {
    clearTimeout(timer);
  }
...
async function parseCredentials(request) {
  ...
  catch { return { ok: false, status: 400, message: 'Invalid JSON body' }; }
...
async function exchangeGeneralSession(request, env, cors) {
  ...
  const canonicalUsername = text(upstream.username || (upstream.user && (upstream.user.username || upstream.user.name)) || credentials.username);
  ...
  return json({ success: true, edgeToken, expiresAt: new Date((now + ttl) * 1000).toISOString(), expiresIn: ttl, user: { username: canonicalUsername }, authSource: 'apps-script-post', sessionBridge: 'cloud-session-bridge-v3-post' }, 200, cors);
}
...
async function exchangeOrdersSession(request, env, cors) {
  ...
  return json({ success: true, edgeToken, expiresIn: ttl, expiresAt: new Date((now + ttl) * 1000).toISOString(), user: { username: canonicalUsername, role, department, screens }, authSource: 'apps-script-post', sessionBridge: 'cloud-session-bridge-v3-post' }, 200, cors);
}
...
export async function handleCloudSessionBridgeV3(request, env) {
  ...
  try {
    return path === ORDERS_SESSION_PATH ? exchangeOrdersSession(request, env, cors) : exchangeGeneralSession(request, env, cors);
  } catch (err) {
