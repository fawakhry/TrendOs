// TrendOS Tasks V3 — branch-only read-only Worker preview adapter.
// This module is intentionally NOT wired into any production entrypoint.

export const TASKS_V3_PROTOCOL = 'TRENDOS_TASKS_V3_READONLY_1';
export const TASKS_V3_PREVIEW_ALLOWED_OPS = Object.freeze([
  'health',
  'status',
  'flyPrint',
  'pressCandidates'
]);
export const TASKS_V3_PREVIEW_ALLOWED_ROLE = 'WAEL';
export const TASKS_V3_PREVIEW_TIMEOUT_MS = 5000;
...
export async function buildTasksV3PreviewAssertion({
  op,
  operator,
  ...
  payload.signature = await hmacSha256Hex(canonicalPayload(payload), normalizedSecret, cryptoImpl);
  return payload;
}
...
export async function proxyTasksV3ReadonlyPreview({
  env,
  op,
  operator,
  role = TASKS_V3_PREVIEW_ALLOWED_ROLE,
  ...
  const boundedTimeout = Math.max(1, Math.min(Number(timeoutMs) || TASKS_V3_PREVIEW_TIMEOUT_MS, TASKS_V3_PREVIEW_TIMEOUT_MS));
  ...
  try {
    ...
    return {
      ...
      code: body && body.success === true ? 'OK' : text(body && body.code) || 'TASKS_V3_PREVIEW_UPSTREAM_REJECTED',
