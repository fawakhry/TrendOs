...
export const T12_SHADOW_STORE_VERSION = 'TRENDOS_T12_ORDER_CREATE_SHADOW_STORE_20260919';
...
export async function persistT12OrderCreateShadow(db,input={},actor='',options={}){
  ...
  const existing=await db.prepare(
    'SELECT client_request_id AS clientRequestId, provisional_ref AS provisionalRef, canonical_json AS canonicalJson, qualification_status AS qualificationStatus FROM t12_order_create_intents WHERE client_request_id = ? LIMIT 1'
  ).bind(intent.requestKey).first();

  if (existing) {
    if (text(existing.canonicalJson)===canonicalJson) {
      return {
  ...
  try {
    ...
    const raced=await db.prepare(
      'SELECT provisional_ref AS provisionalRef, canonical_json AS canonicalJson, qualification_status AS qualificationStatus FROM t12_order_create_intents WHERE client_request_id = ? LIMIT 1'
    ).bind(intent.requestKey).first();
    if (raced && text(raced.canonicalJson)===canonicalJson) {
      return {
        success:true, stored:false, idempotent:true, conflict:false,
        productionCutoverAuthorized:false, businessOrderIdAllocated:false,
        version:T12_SHADOW_STORE_VERSION, requestKey:intent.requestKey,
