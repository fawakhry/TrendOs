...
class FakeStatement {
  ...
  bind(...args) { this.args = args; this.db.calls.push({ sql: this.sql, args }); return this; }
  async first() { return this.db.firstRow; }
  async run() { this.db.runCount += 1; return { success: true }; }
}
class FakeDb {
  constructor(firstRow = null) { this.firstRow = firstRow; this.calls = []; this.runCount = 0; }
...
}
{
  ...
  assert.equal(lookup.hit, false); assert.equal(lookup.reason, 'disabled');
  assert.equal(stored.stored, false); assert.equal(stored.reason, 'disabled');
...
{
  const db = new FakeDb({
    canonicalUsername: 'wael', role: 'print', department: 'طباعة',
    screensJson: '["print","press",""]', verifiedAtMs: 1000,
    expiresAtMs: 999999, source: 'apps-script-post'
  });
  const result = await lookupCloudAuthShadow('wael', 'employee-secret-token', env(db), 2000);
...
console.log('CLOUD_AUTH_SHADOW_V1_T6B=PASS');
