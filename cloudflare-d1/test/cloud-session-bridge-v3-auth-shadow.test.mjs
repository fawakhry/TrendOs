...
class FakeStatement {
  ...
  async first() { this.db.firstCount += 1; return this.db.firstRow; }
  async run() { this.db.runCount += 1; return { success: true }; }
}
class FakeDb {
  constructor(firstRow = null) { this.firstRow = firstRow; this.calls = []; this.firstCount = 0; this.runCount = 0; }
...
try {
  {
    ...
    let fetchCalls = 0;
    globalThis.fetch = async () => { fetchCalls += 1; throw new Error('Apps Script must not be called on auth shadow hit'); };
  ...
  }
  {
    ...
    globalThis.fetch = async (input, init = {}) => {
      ...
      return new Response(JSON.stringify({ success: true, user: { username: 'wael', role: 'print', department: 'طباعة' } }), {
  ...
  console.log('CLOUD_SESSION_BRIDGE_V3_AUTH_SHADOW_T6B=PASS');
