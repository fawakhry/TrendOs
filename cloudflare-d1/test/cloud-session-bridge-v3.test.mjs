...
async function testPostTransportDoesNotLeakTokenInUrl() {
  ...
  assert.equal(typeof payload._ts, 'number');
}

async function testGeneralSessionExchange() {
  const calls = mockFetch({ success: true, username: 'wael' });
  const response = await handleCloudSessionBridgeV3(
    request('/v1/edge/session', { username: 'wael', token: 'employee-secret-token' }),
    env()
  );
...
async function testOrdersSessionExchange() {
  ...
  const response = await handleCloudSessionBridgeV3(
    request('/v1/edge/orders/session', { username: 'wael', token: 'employee-secret-token' }),
    env()
  );
  const body = await response.json();
  assert.equal(response.status, 200);
...
try {
  ...
  console.log('CLOUD_SESSION_BRIDGE_V3_T1=PASS');
