/**
 * T12 PACKED-128 synthetic-only planner — NO IO, no D1 POST, no Script Properties,
 * no Sheet write, no deploy. All row payloads remain in process memory.
 * NEVER IMPORT INTO PRODUCTION: no deployment, no live I/O, no write permission.
 * A valid isolated plan does NOT prove production quota, runtime or CAS behavior.
 */
const NAMES = ['الأوردرات', 'بنود الأوردرات'];
const NOTE = 'TrendOS orders live sync V2 quota-aware';
const MAX_ROWS_PER_TAB = 5000;
const MAX_CANDIDATES = 160; // ISOLATED DESIGN ONLY; no live write authorization
const MAX_TAIL_GROWTH_PER_TAB = 64; // isolated design bound; NOT R4 5-row live bound
const MAX_PAYLOAD_BYTES = 250000; // hypothetical private-proposal ceiling, not actual measured bytes
...
function validateMirror(src, mirror) {
  ...
  assert(Array.isArray(mirror.rows) && mirror.rows.length === cat.rowCount &&
    ...
    'MIRROR_ROWS');
  // The separate ISOLATED planner permits a bounded tail larger than R4's live 5.
  // Do not reuse its elevated ceiling in any currently deployed path.
  assert(src.sourceLastRow - cat.rowCount <= MAX_TAIL_GROWTH_PER_TAB,
    'ISOLATED_TAIL_GROWTH_LIMIT');
  return cat;
}
 ...
 */
export function buildIsolatedPacked128RecoveryPlan({ sourceTabs, mirrorTabs,
...
export function publicIsolatedPacked128Summary(plan) {
