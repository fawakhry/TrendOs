(function(){
  ...
  function fit(root,left,top){
    const w=root.offsetWidth||350,h=root.offsetHeight||110;
  ...
  function bindHandle(root,handle){
    if(!handle||handle.dataset.dragHandleV2==='1')return;
    handle.dataset.dragHandleV2='1';handle.style.touchAction='none';handle.style.userSelect='none';handle.title='اسحب اللوحة لمكان مناسب — لا يمكن إغلاقها';
    ...
    function end(e){if(!active||e.pointerId!==pid)return;active=false;try{handle.releasePointerCapture(pid);}catch(err){}if(moved){suppress=true;save(root);setTimeout(function(){suppress=false;},160);}pid=null;}
  ...
  function attach(root){
    if(!root)return;
    restore(root);
    root.querySelectorAll('[data-strip],.ems-panel-head').forEach(function(handle){bindHandle(root,handle);});
    if(root.dataset.dragV2Resize!=='1'){
      root.dataset.dragV2Resize='1';
      window.addEventListener('resize',function(){const r=root.getBoundingClientRect();apply(root,{left:r.left,top:r.top});save(root);});
    }
  }
  const t=setInterval(function(){const root=document.getElementById('employeeManagerStripsV2');if(!root)return;attach(root);clearInterval(t);},200);
})();
