...
function normPhone_(phone){ return String(phone || "").replace(/\\s+/g,"").replace(/-/g,"").replace(/\\+/g,"").replace(/^20/,"0").replace(/^2/,"0").trim(); }
...
function ensureUsersSetup_(){
  ...
  if(!h["Token"]){
    sheet.getRange(1, sheet.getLastColumn()+1).setValue("Token");
  }

  h = headersMap_(sheet);

  if(!h["آخر دخول"]){
    sheet.getRange(1, sheet.getLastColumn()+1).setValue("آخر دخول");
  }
}
...
function createManualOrder_(e){
  ...
  const assignedTo = normalize_(e.parameter.assignedTo) || defaultAssigned_(department);
  const notes = normalize_(e.parameter.notes);

  if(!customerName || !customerPhone || !department || !itemName){
    return { success:false, message:"بيانات الأوردر ناقصة." };
  }
  ...
  appendOrder_(ss, { orderId, now, customerName, customerPhone, customerType, department, itemName, qty, priority, status, assignedTo, notes });
  appendLine_(ss, { orderId, lineId, now, customerName, customerPhone, customerType, department, itemName, qty, priority, status, assignedTo, notes });

  return { success:true, orderId, lineId, message:"تم إضافة الأوردر." };
}
