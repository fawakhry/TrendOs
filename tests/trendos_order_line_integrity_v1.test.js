...
class Range{constructor(sh,r,c,nr=1,nc=1){Object.assign(this,{sh,r,c,nr,nc})}getValues(){const out=[];for(let i=0;i<this.nr;i++){const row=[];for(let j=0;j<this.nc;j++)row.push(this.sh.getCell(this.r+i,this.c+j));out.push(row)}return out}}
class Sheet{constructor(headers,rows){this.headerNames=headers;this.headers={};headers.forEach((h,i)=>this.headers[h]=i+1);this.rows=[headers,...rows.map(r=>r.slice())]}getLastRow(){return this.rows.length}getLastColumn(){return this.headerNames.length}getRange(r,c,nr=1,nc=1){return new Range(this,r,c,nr,nc)}getDataRange(){return new Range(this,1,1,this.rows.length,this.headerNames.length)}getCell(r,c){return (this.rows[r-1]||[])[c-1]??''}setCell(r,c,v){while(this.rows.length<r)this.rows.push([]);while(this.rows[r-1].length<c)this.rows[r-1].push('');this.rows[r-1][c-1]=v}}
