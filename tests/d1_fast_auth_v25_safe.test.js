...
const a1=ctx.authorizeD1FastV25_('Diaa',token);assert.strictEqual(a1.ok,true);assert.strictEqual(a1.cacheHit,false);assert.strictEqual(authCalls,1);assert.strictEqual(a1.user.username,'diaa');assert.strictEqual(a1.user.password,undefined);assert.strictEqual(a1.user.token,undefined);assert.strictEqual(ttls[0],120);
const raw=[...cache.values()].join('\n');assert.ok(!raw.includes(token));assert.ok(!raw.includes('HASH-SECRET'));assert.ok(!/"password"\s*:/.test(raw));assert.ok(!/"token"\s*:/.test(raw));
const keys=[...cache.keys()].join('\n');assert.ok(!keys.includes(token));assert.ok(!keys.toLowerCase().includes('diaa'));
const a2=ctx.authorizeD1FastV25_('Diaa',token);assert.strictEqual(a2.ok,true);assert.strictEqual(a2.cacheHit,true);assert.strictEqual(a2.authSource,'FAST_AUTH_CACHE_V25');assert.strictEqual(authCalls,1);
const rev1=ctx.trendosFastAuthInvalidateUserV25_('Diaa');assert.strictEqual(rev1,'1');const a3=ctx.authorizeD1FastV25_('Diaa',token);assert.strictEqual(a3.ok,true);assert.strictEqual(a3.cacheHit,false);assert.strictEqual(authCalls,2);
allowAuth=false;ctx.trendosFastAuthAfterLogoutV25_('Diaa');const denied=ctx.authorizeD1FastV25_('Diaa',token);assert.strictEqual(denied.ok,false);assert.strictEqual(denied.message,'revoked');assert.strictEqual(authCalls,3);
allowAuth=true;expiredValue='2026-08-30T10:00:00Z';ctx.trendosFastAuthAfterLoginV25_('Diaa');const expired=ctx.authorizeD1FastV25_('Diaa',token);assert.strictEqual(expired.ok,false);assert.strictEqual(authCalls,4);expiredValue='';
...
assert.ok(src.includes('Direct manual edits to Users Active/password/token fields'));assert.ok(!/sk-[A-Za-z0-9_-]{20,}/.test(src));assert.ok(!/EAA[A-Za-z0-9]{30,}/.test(src));
