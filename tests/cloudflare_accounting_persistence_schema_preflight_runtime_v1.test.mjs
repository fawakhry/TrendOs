...
test('missing explicit preview D1 fails closed', async () => {
  ...
  assert.equal(body.code, 'D1_NOT_INJECTED');
...
test('explicit preview D1 performs metadata reads only and reports compatible schema', async () => {
  const spy = compatibleD1Spy();
  const { response, body } = await request({ TRENDOS_ACCOUNTING_PREVIEW_DB: spy.db });
  assert.equal(response.status, 200);
  assert.equal(body.success, true);
  assert.equal(body.code, 'SCHEMA_COMPATIBLE');
  assert.equal(body.compatible, true);
  assert.equal(body.readOnly, true);
  assert.equal(body.authoritativeWrites, false);
  assert.equal(body.mutationPerformed, false);
  assert.equal(spy.calls.run, 0);
  assert.equal(spy.calls.batch, 0);
  assert.ok(spy.calls.prepare > 0);
  assert.ok(spy.calls.all > 0);
});
...
test('non-GET schema preflight is rejected without touching D1', async () => {
  ...
  assert.equal(body.code, 'accounting-persistence-schema-preflight-read-only');
  assert.equal(body.authoritativeWrites, false);
  assert.equal(body.mutationPerformed, false);
