...
assert.deepEqual(
  db.prepare("SELECT account_code, role FROM accounting_accounts WHERE account_code IN ('1010','1100','2100','4100') ORDER BY account_code").all(),
  [
    { account_code: '1010', role: 'cash-main' },
    { account_code: '1100', role: 'accounts-receivable' },
    { account_code: '2100', role: 'accounts-payable' },
    { account_code: '4100', role: 'sales-revenue' }
  ]
);
...
assert.deepEqual(dimensions, {
  party_id: 'CUST-TEST-1',
  treasury_id: 'CASHBOX-BENHA-MAIN',
  order_id: 'ORDER-TEST-1',
  line_id: 'LINE-TEST-1',
  department_id: 'DEPT-PRINT',
  profit_center_id: 'PC-PRINT'
});
...
assert.equal(db.prepare("SELECT status FROM accounting_idempotency WHERE idempotency_key='IDEMP-AMB-1'").get().status, 'ambiguous');
...
for (const [sqlText, message] of [
  ...
  assert.throws(() => db.exec(sqlText), new RegExp(message.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')));
}
...
assert.equal(db.prepare("SELECT status FROM accounting_journals WHERE journal_id='JRN-TEST-1'").get().status, 'posted');
assert.equal(db.prepare("SELECT reversal_of_journal_id FROM accounting_journals WHERE journal_id='JRN-REV-1'").get().reversal_of_journal_id, 'JRN-TEST-1');
