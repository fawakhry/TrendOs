 ...
 * نسخة كاملة موحدة V1831: TrendOS + Matbagy Bridge + Pricing Fix:
 * 1) تسجيل الدخول
...
function createCustomer_(e) {
  ...
  SpreadsheetApp.flush();
  return { success: true, message: "تم إضافة العميل في شيت العملاء.", customer: { name: customerName, manager: manager, phone: phone, extraPhone: extraPhone, type: customerType, active: active, debtAmount: debtAmount } };
}
...
/************************************************************
 * V1833 PATCH - إشعارات العاجل + فلتر مكبس مستقل
...
function getUrgentNotifications_(e) {
  ...
  const rows = (res.rows || []).filter(function(row) {
    const priority = normalize_(row.priority);
