...
function getRows_(e) {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    rows.push({
      ...
      receivedAt: valueAt_(row, colReceivedAt),
      expectedDeliveryAt: valueAt_(row, colExpectedAt),
      expectedDeliveryText: normalize_(valueAt_(row, colExpectedText)) || (valueAt_(row, colExpectedAt) ? formatDateAr_(valueAt_(row, colExpectedAt)) : ""),
