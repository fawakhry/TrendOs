(function(){
  'use strict';
  if (window.__TREND_MASTER_RESILIENCE_SAFE_V1931__) return;
  window.__TREND_MASTER_RESILIENCE_SAFE_V1931__ = true;
  ...
  var CORE_PANELS = ['summary','archive','employee','stock','messages','debt'];
  var ALL_PANELS = CORE_PANELS.concat(['dayclose']);
  var defaultTimeouts = {summary:30000,archive:60000,messages:30000,stock:30000,employee:30000,debt:30000,dayclose:120000};
  var maxConcurrency = Math.max(1, Math.min(2, Number(window.MATBAGY_TREND_MASTER_MAX_CONCURRENCY || 2) || 2));
  var maxAttempts = Math.max(1, Math.min(1, Number(window.MATBAGY_TREND_MASTER_MAX_ATTEMPTS || 1) || 1));
  ...
  function timeoutFor(panel){
    ...
    return n>0?n:defaultTimeouts[panel]||30000;
  }
  ...
  function acquireSlot(){
    if(activeRequests < maxConcurrency){ activeRequests++; return Promise.resolve(); }
    return new Promise(function(resolve){ waiters.push(resolve); }).then(function(){ activeRequests++; });
  }
  ...
  async function requestPanel(panel,params){
    await acquireSlot();
    var url=endpoint();
    if(!url){ releaseSlot(); throw new Error('رابط API غير مضبوط.'); }
    ...
    try {
      var r=await fetch(url,{method:'POST',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify(body),signal:controller.signal,credentials:'omit',redirect:'follow',cache:'no-store'});
      ...
      if(err&&err.name==='AbortError') throw new Error('انتهت مهلة تحميل القسم.');
      throw err;
    } finally {
      clearTimeout(timer);
      releaseSlot();
  ...
  function readPanel(panel,params,options){
    ...
    var attempts=Math.max(1,Math.min(maxAttempts,Number(options.maxAttempts||maxAttempts)||maxAttempts));
  ...
  function renderPanel(panel,outcome){
    ...
    if(panel==='summary'){
      var el=$('trendMasterSummary'),s=d.system||{}; if(!el)return;
      if(!d.system){el.innerHTML=errorHtml(panel,outcome&&outcome.error&&outcome.error.message);return;}
      el.innerHTML=staleBadge(outcome)+dashItem('بنود التشغيل',s.activeLines||0,'')+dashItem('بنود الأرشيف',s.archivedLines||0,'done')+dashItem('تقسيم الصفحات',s.pagingEnabled?'مفعل':'-','ready')+dashItem('سياسة التسليم',s.deliveryPolicy||'-','ready')+dashItem('خصم المخزون',s.stockAutoDeduct||'-','ready'); return;
    }
    if(panel==='archive'){
      var list=$('archiveOrdersList'),pageEl=$('archivePagerText'),a=d.archive||{},rows=a.rows||[],pg=a.pagination||{},allowed=!(d.permissions&&d.permissions.canManageArchive===false); if(!list)return;
      ...
      var prev=$('archivePrevBtn'),next=$('archiveNextBtn');if(prev)prev.disabled=!allowed||(pg.page||1)<=1;if(next)next.disabled=!allowed||(pg.page||1)>=(pg.totalPages||1); return;
    }
    if(panel==='messages'){
      ...
      ml.innerHTML=staleBadge(outcome)+(!msgs.length?'<div class="dash-empty">لا توجد رسائل أو تنبيهات معلقة.</div>':msgs.map(function(msg){var acts=(msg.whatsappUrl?'<button class="primary open-automation-wa" data-message-id="'+esc(msg.id)+'" data-wa-url="'+esc(msg.whatsappUrl)+'" type="button">فتح واتساب</button>':'')+'<button class="ghost copy-automation-message" data-message="'+esc(msg.message)+'" type="button">نسخ الرسالة</button>';return listItem((msg.type||'تنبيه')+(msg.orderId?' — '+msg.orderId:''),[msg.customer,msg.department,msg.assignedTo,msg.status].filter(Boolean).join(' — '),acts,'');}).join('')); return;
    }
    if(panel==='stock'){
      ...
      sl.innerHTML=staleBadge(outcome)+(!stocks.length?'<div class="dash-empty">المخزون أعلى من حدود التنبيه.</div>':stocks.map(function(item){return listItem(item.material,item.department+' — الرصيد: '+item.stock+' — الحد الأدنى: '+item.minimum,'','<span class="trend-score warn">منخفض</span>');}).join('')); return;
    }
    if(panel==='employee'){
      ...
      kl.innerHTML=staleBadge(outcome)+(!ks.length?'<div class="dash-empty">لا توجد بيانات تقييم اليوم.</div>':ks.map(function(k){return listItem(k.employee+' — '+(k.department||''),(k.orderCount||0)+' أوردر / '+(k.total||0)+' بند — منجز '+(k.completed||0)+' — متأخر '+(k.overdue||0)+' — إنجاز '+(k.completionPercent||0)+'%','','<span class="trend-score '+(Number(k.score)<60?'warn':'')+'">'+esc(k.score||0)+'%</span>');}).join('')); return;
    }
    if(panel==='debt'){
      var dl=$('debtRestrictionList'),opts=$('debtRestrictionCustomers'),dc=d.debtControl||{},cs=dc.customers||[],rs=dc.restrictions||[]; if(d.permissions&&d.permissions.canManageDebtRestrictions===false)return;if(!dl)return;
  ...
  function setDaycloseIdle(){
    var el=$('dayClosePreviewList');if(!el)return;
    if(latest.dayclose&&latest.dayclose.data){renderPanel('dayclose',latest.dayclose);return;}
    el.innerHTML='<div class="dash-empty">قفلة اليوم لا تُحسب تلقائيًا لتخفيف الحمل على السيرفر. <button type="button" class="ghost tmr-retry" data-tmr-retry="dayclose">مراجعة قفلة اليوم</button></div>';
    var badge=$('dayCloseReadyBadge');if(badge)badge.textContent='يدوي';
  }
  function updateOverallStatus(){
    var status=$('trendMasterStatus');if(!status)return;
    var failed=CORE_PANELS.filter(function(p){return latest[p]&&latest[p].ok===false&&!latest[p].stale;});
    var stale=CORE_PANELS.filter(function(p){return latest[p]&&latest[p].stale;});
    var pending=CORE_PANELS.filter(function(p){return !latest[p];});
    if(failed.length)status.textContent='تحديث جزئي — تعذر '+failed.length+' قسم — '+nowText();
    else if(pending.length)status.textContent='جاري تحميل '+pending.length+' قسم — '+nowText();
    else if(stale.length)status.textContent='تم التحديث مع '+stale.length+' قسم ببيانات محفوظة — '+nowText();
    else status.textContent='آخر تحديث: '+nowText();
  }
  function reapplyPending(){
    CORE_PANELS.forEach(function(p){if(latest[p])renderPanel(p,latest[p]);else setPanelLoading(p);});
    setDaycloseIdle(); updateOverallStatus();
  }
  function launchBatch(params,forceLoading){
    ...
    if(forceLoading!==false)CORE_PANELS.forEach(setPanelLoading);
    setDaycloseIdle();
    var jobs=CORE_PANELS.map(function(panel){return readPanel(panel,params,{maxAttempts:1}).then(function(out){renderPanel(panel,out);updateOverallStatus();return out;});});
  ...
  function compatibleSnapshot(){
    var out={success:true,system:{},employeePerformance:[],stockAlerts:[],messageQueue:[],archive:{success:true,rows:[],pagination:{page:1,pageSize:10,totalRows:0,totalPages:1}},debtControl:{customers:[],restrictions:[]},dayClose:null,permissions:{},version:'V1931_TREND_MASTER_RESILIENCE_SAFE_V1'};
    ALL_PANELS.forEach(function(p){var d=latest[p]&&latest[p].data;if(!d)return;if(d.system)out.system=d.system;if(d.employeePerformance)out.employeePerformance=d.employeePerformance;if(d.stockAlerts)out.stockAlerts=d.stockAlerts;if(d.messageQueue)out.messageQueue=d.messageQueue;if(d.archive)out.archive=d.archive;if(d.debtControl)out.debtControl=d.debtControl;if(d.dayClose)out.dayClose=d.dayClose;if(d.permissions)out.permissions=Object.assign(out.permissions,d.permissions);});
    return out;
  }
  async function interceptedApi(action,params){
    ...
    var summary=latest.summary || await readPanel('summary',context,{maxAttempts:1});
    renderPanel('summary',summary);
    var snapshot=compatibleSnapshot();
    setTimeout(reapplyPending,0);
    return snapshot;
  }
  function retry(panel){
    if(ALL_PANELS.indexOf(panel)===-1)return Promise.resolve(null);
    var params=lastContext||cleanContext({}); setPanelLoading(panel);
    return readPanel(panel,params,{maxAttempts:1}).then(function(out){renderPanel(panel,out);updateOverallStatus();return out;});
  }
  function inject(){
    if(!document.getElementById('trendMasterResilienceSafeV1Style')){var s=document.createElement('style');s.id='trendMasterResilienceSafeV1Style';s.textContent='.tmr-error{color:#b42318}.tmr-stale{margin:0 0 8px;padding:7px 9px;border-radius:9px;background:#fff7ed;color:#9a3412;font-size:12px}.tmr-retry{margin-inline-start:8px}.tmr-loading{opacity:.75}';(document.head||document.documentElement).appendChild(s);}
    var root=$('trendMasterCenter');if(root&&!root.__tmrSafeBound){root.__tmrSafeBound=true;root.addEventListener('click',function(ev){var b=ev.target&&ev.target.closest&&ev.target.closest('[data-tmr-retry]');if(b){ev.preventDefault();retry(b.getAttribute('data-tmr-retry'));}});}
  }
  function install(){
    ...
    inject();
    window.trendMasterPanelResilienceV1={readPanel:readPanel,retry:retry,refresh:function(params){return launchBatch(params||lastContext||{},true);},snapshot:compatibleSnapshot,_debug:{inflight:inflight,lastGood:lastGood,latest:latest,timeoutFor:timeoutFor,getActive:function(){return activeRequests;},getQueued:function(){return waiters.length;},maxConcurrency:maxConcurrency,maxAttempts:maxAttempts}};
    return true;
  }
  if(!install()){
    var tries=0,t=setInterval(function(){tries++;if(install()||tries>=200)clearInterval(t);},50);
  }
})();
