...
function saveAccountingTemplate_(e) {
  ...
  const values = {
    ...
    "تكلفة ثابتة": parseMoney_(e.parameter.fixedCost || e.parameter.cost || e.parameter.unitCost),
  ...
  if (existingRow) {
    ...
    delete values["وقت التسجيل"];
    updateByHeaders_(sheet, existingRow, values, true);
    return { success: true, message: "الصنف موجود وتم تحديثه بدل إضافته مرة أخرى.", updated: true, rowNumber: existingRow, version: "V1877_ES30" };
  }
  appendByHeaders_(sheet, values);
  return { success: true, message: "تم حفظ الصنف.", updated: false, version: "V1877_ES30" };
}
...
function saveAccountingDeptLine_(e) {
  ...
  appendByHeaders_(sheets.deptLines, {
    ...
    "اسم البند": itemName,
    "كود الصنف": normalize_(e.parameter.catalogItemId || e.parameter.templateId || e.parameter.itemId),
    "الكمية": qty,
    "الخامة": materialName,
    "استهلاك الخامة": qty,
...
function createCustomer_(e) {
  ...
  return { success: true, message: "تم إضافة العميل في شيت العملاء." + (portalCode ? " كود الشات: " + portalCode + " | كلمة المرور المؤقتة: " + CUSTOMER_DEFAULT_PASSWORD : ""), customer: { name: customerName, manager: manager, phone: phone, extraPhone: extraPhone, type: customerType, active: active, debtAmount: debtAmount, branchCode: branchCode, branchName: branchName, customerCode: portalCode, defaultPassword: CUSTOMER_DEFAULT_PASSWORD } };
}
...
function createCustomer_(e) {
  ...
  return {
    success: true,
    message: "تم إضافة العميل في شيت العملاء." + (portalCode ? " كود الشات: " + portalCode + " | كلمة المرور المؤقتة: " + CUSTOMER_DEFAULT_PASSWORD : ""),
    updated: false,
    customer: { name: customerName, manager: manager, phone: phone, extraPhone: extraPhone, type: customerType, active: active, debtAmount: debtAmount, branchCode: branchCode, branchName: branchName, customerCode: portalCode, defaultPassword: CUSTOMER_DEFAULT_PASSWORD }
  };
}
...
function getAccounting_(e) {
  ...
  return {
    ...
    version: "V1857_ES14_ACCOUNTING_MERGE"
  };
}
function saveEasyStorePurchase_(e) {
  const auth = accountingAuthorize_(e);
...
function saveEasyStoreSaleV2_(e) {
  ...
  return { success:true, message:"تم حفظ فاتورة البيع وتحديث حساب العميل." };
}
function saveEasyStorePurchase_(e) {
  const auth = accountingAuthorize_(e);
  if (!auth.ok) return { success: false, message: auth.message };
  ...
  try { saveStockMoveBatch25_(normalize_(e.parameter.materialName || e.parameter.material), qty, 0, "شراء", invoiceNo, auth.user.username); } catch(err) {}
  ...
  return { success:true, message:"تم حفظ فاتورة الشراء وتحديث حساب المورد." };
}
...
function easyStoreSystemHealth_(e) {
  ...
  return { success:true, message:"النظام سليم", sheets:Object.keys(sh).concat(["حسابات - كشف العملاء والموردين","حسابات - الخزنة","حسابات - سجل المراجعة","حسابات - قفلة اليوم"]), version:"V1861 ES18 Error Fix + Zero Reset", permissions:{ mode:auth.mode, canEnterPurchaseInvoice: accountingCanSavePurchaseV1857_(auth), canSeeCosts: auth.mode === "full", canEditLedger:accountsCanEditV1858_(auth), canClearMaterials:auth.mode==="full" } };
}
...
function saveAccountingFinalInvoice_(e) {
  ...
  try {
    ...
    SpreadsheetApp.flush();
    return { success: true, message: "تم تقفيل الفاتورة النهائية رقم " + invoiceNo + " من البنود المعتمدة فقط.", invoiceNo: invoiceNo, lineCount: rowsToClose.length, subtotal: subtotal, finalTotal: finalTotal, paid: paid, remaining: remaining, trustedByServer: true, version: "V1889_STABLE_MERGE" };
  } finally {
    try { lock.releaseLock(); } catch (err) {}
  }
}
