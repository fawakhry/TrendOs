/* TrendOS V1874 Clean Login Fix - fixes employee login without changing business screens */
(function(){
  'use strict';
  var VERSION = 'V1874 Clean Login Fix';
  ...
  function openAfterLogin(user){
    ...
    url.searchParams.set('v','trendos-v1874-clean-login-fix-' + Date.now());
  ...
  function bind(){
    ...
    var user = $('username');
    var pass = $('password');
    if(btn && btn.dataset.v1874LoginRescue !== '1'){
      btn.dataset.v1874LoginRescue = '1';
      btn.addEventListener('click', rescueLogin, true);
      btn.onclick = rescueLogin;
    }
    if(user && user.dataset.v1874Enter !== '1'){
      user.dataset.v1874Enter = '1';
      user.addEventListener('keydown', function(e){ if(e.key === 'Enter'){ e.preventDefault(); if(pass) pass.focus(); } }, true);
    }
    if(pass && pass.dataset.v1874Enter !== '1'){
      pass.dataset.v1874Enter = '1';
    ...
    if(badge && /V1856|V186|V187|Batch|ES/.test(badge.textContent || '')){
      badge.textContent = 'TrendOS V1874 - Clean Login Fix';
    }
  }
  ...
  window.trendosLoginRescueReady = true;
})();
