...
function renderManager(m){const r=ensureRoot();if(!r)return;const rows=m&&m.employees||[];r.innerHTML=css()+'<div style="display:flex;justify-content:space-between"><b>📊 أداء التاسكات</b><button data-ot="metrics" class="ghost">تحديث</button></div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:8px;margin-top:10px">'+rows.map(function(x){return '<div style="border:1px solid #edf1f4;border-radius:10px;padding:10px"><b>'+esc(x.employee)+'</b><div>'+Number(x.completedOrders||0)+' أوردر • '+Number(x.completedTasks||0)+' تاسك</div><small>إجمالي '+fmtSec(x.totalWorkSec)+' • متوسط '+fmtSec(x.averageWorkSec)+'</small></div>';}).join('')+'</div>';}
async function refresh(){if(busy||!user())return;busy=true;try{const d=await api('status');if(!d||!d.success||d.enabled!==true){last=d;return;}if(d.role==='MANAGER'){const m=await api('metrics');if(m&&m.success)renderManager(m);return;}renderEmployee(d);}catch(e){console.warn('Operator Task V2:',e);}finally{busy=false;}}
...
async function onClick(e){const b=e.target&&e.target.closest?e.target.closest('[data-ot]'):null;if(!b)return;const a=b.getAttribute('data-ot');if(a==='refresh'){await refresh();return;}if(a==='claim'){const d=await act('claimNext');if(d)await refresh();return;}if(a==='ready'||a==='delivered'){if(!last||!last.task)return;const finalStatus=a==='ready'?'جاهز للاستلام':'تم التسليم';if(!confirm('تأكيد تحويل التاسك إلى '+finalStatus+'؟'))return;const d=await act('completeTask',{taskId:last.task.taskId,finalStatus:finalStatus});if(d){alert('الوقت الفعلي: '+fmtSec(d.actualWorkSec||d.task&&d.task.workSec));await refresh();}return;}if(a==='pressToggle'){if(pressOpen){pressOpen=false;pressItems=[];if(last)renderEmployee(last);return;}const d=await act('pressCandidates');if(d){pressItems=Array.isArray(d.items)?d.items:[];pressOpen=true;if(last)renderEmployee(last);}return;}if(a==='metrics'){const m=await act('metrics');if(m)renderManager(m);}}
