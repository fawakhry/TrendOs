...
## RP-06-WRITER-J — PROJECT MEMORY ADVANCED TO HEAD/PREVIEW BOUNDARY

- **Action:** Updated `TRENDOS_PROJECT_MEMORY.md` with the hardened writer source/blob, revised reader source/blob, 34-row plan hash, Invoice row pins, CI, safety controls, uninstalled/unwritten status, and the exact bounded Head-install/read-only-preview next gate.
- **Evidence:** Memory commit `f98753ecd33f68bb07c7c6a272069f86bb031ee7`; blob `bb8e16512909eac1533804e239043ccb803f4b59`.
- **Status:** PASS.
- **Production impact:** NONE. Documentation-only; Version 146 remains live and unchanged.
...
## RP-06-PREVIEW-RERUN-A — OBSERVABLE WRITER INSTALLED AND EXACT-VERIFIED IN APPS SCRIPT HEAD

- **Action:** Under the user's explicit `RP-06-PREVIEW-RERUN` approval, replaced only the content of Apps Script Head file `trendos-core-p0-registry-writer-v1.gs` with the observable/fail-closed source from commit `47d793809a609aadcfb61279180744fcf27c5c99`; saved the project, allowed Apps Script to parse it, reloaded the editor, reselected the same file, copied the complete persisted content, and verified its Git blob exactly.
- **Evidence:** Pre-replacement persisted Head blob `92a9fc442031d1c48c295c81bceb372e8c9f89d8`, length 28,305 characters. Git source and staged editor content both matched blob `f8e1a4be7b9de3ea1dcbda7a17c87e483ea8ecc4`. After Save and full Reload, persisted content remained an exact match to blob `f8e1a4be7b9de3ea1dcbda7a17c87e483ea8ecc4`, length 28,616 characters / 28,840 UTF-8 bytes; `cloud_done` was visible, Save was disabled, and no syntax/duplicate-global/parse error was surfaced.
- **Status:** PASS — the approved observable writer is saved, parsed, reloaded, and exact-verified in Apps Script Head.
- **Production impact:** Apps Script Head source only. Production Web App remains Version 146. No Deploy, Registry Sheet/row, Script Property, feature flag, trigger, route, source Sheet, Fast Auth, `Code.gs`, or business data change occurred, and no writer/rollback function was run.
- **Commit / CI:** Source/test commit `47d793809a609aadcfb61279180744fcf27c5c99`; writer blob `f8e1a4be7b9de3ea1dcbda7a17c87e483ea8ecc4`; test blob `bff68556aca2f7e8e32ef6055924de5fc3895f43`; CI `33619745373` SUCCESS. Local dedicated writer test also passed immediately before Head replacement.
- **Rollback:** Replace only the Head writer with prior blob `92a9fc442031d1c48c295c81bceb372e8c9f89d8` if an explicit Head rollback is later required. No production deployment rollback is applicable because Version 146 was not changed.
- **Exact next step:** Run only `trendosCoreP0RegistryPreviewV1`; capture the complete logged JSON and require `success=true`, `readOnly=true`, version `TRENDOS_CORE_P0_REGISTRY_WRITER_V1_20260901`, plan hash `5e80dd09271d21e96e3f415c21688e7f16bcac2f4b664cc23d38b08c1036aa29`, `expectedCount=34`, `actualPlanCount=34`, `errors=[]`, exactly 34 checks, and every check valid with empty errors and matching expected/actual hashes. On any mismatch record FAIL and stop without Registry Write or any other production mutation.


## RP-06-PREVIEW-RERUN-B — READ-ONLY OBSERVABLE PREVIEW FAIL; STOP BEFORE REGISTRY WRITE

- **Action:** Ran only `trendosCoreP0RegistryPreviewV1` once from the exact-verified observable writer and inspected the Apps Script execution log. No write or rollback function was selected or invoked.
- **Evidence:** The logged result exposed `success=false`, `readOnly=true`, version `TRENDOS_CORE_P0_REGISTRY_WRITER_V1_20260901`, plan hash `5e80dd09271d21e96e3f415c21688e7f16bcac2f4b664cc23d38b08c1036aa29`, `expectedCount=34`, and `actualPlanCount=34`. Top-level `errors` contains exactly three failures for `PRESS_COMPLETED_WITHOUT_SESSION / 3536-01`: `source row count 2 != 1`, `live evidence hash mismatch`, and `Line is no longer an eligible Press completion`. Its visible check has expected hash `02ec63d746d1bda0f3d1505ac807c3e0baaeb3188c194ed0b5c24d8704796293`, actual hash `5bb4c887de84bc503c85360ae21ed04e92a553c53c65543d4a52f8a6570d684a`, `valid=false`, and the same three check errors. Apps Script reported `Logging output too large. Truncating output.` after emitting the beginning of the JSON, then the public wrapper threw `CORE-P0 registry preview failed` at writer line 152 with those three errors. The execution therefore ended as an Error rather than a false PASS.
- **Status:** FAIL — the preview does not satisfy the approved PASS contract. The full 34-check JSON was not retrievable from the editor because Apps Script truncated the oversized log, but the observable top-level failure and invalid `3536-01` check are sufficient and authoritative to fail the gate.
- **Production impact:** READ-ONLY preview only. Production Web App remains Version 146. No Registry Sheet/row was created or changed, no Script Property was created or modified, and no Deploy, feature flag, trigger, route, source Sheet, Fast Auth, `Code.gs`, or business-data mutation occurred. `trendosCoreP0RegistryWriteV1` and `trendosCoreP0RegistryRollbackV1` were not run.
...
## PERF-CF-01 — VERSION-146 LIVE PERFORMANCE BASELINE AND CLOUDFLARE PATH DIAGNOSIS

- **Action:** Began the user-approved production-performance/Cloudflare lane with read-only evidence only. Inspected the live Apps Script Version-146 execution table, then reconciled the observed timings against the existing frontend request schedule, Apps Script/D1 read path, and Cloudflare Worker routes. The paused CORE-P0 remediation lane was not resumed.
- **Evidence:** The live completed snapshot contained 10 `doGet` executions averaging 26.247s (median 11.345s, p90 44.519s, max 113.306s), 35 `doPost` executions averaging 24.955s (median 17.267s, p90 53.591s, max 74.301s), and 5 `d1OrdersLiveSyncTick` executions averaging 14.169s with a 39.669s maximum. The real-time table also exposed 32 concurrently running rows, including one `doGet` over 208s. Source reconciliation shows `config.js` still routes `WEB_APP_URL`, `TREND_API_URL`, and `API_URL` directly to the Apps Script deployment while `MATBAGY_SECURE_API_PROXY_URL` is empty; existing D1 reads therefore still pay Apps Script startup and legacy authorization before reaching D1/cache. Existing read-path evidence identifies synchronous full-`Users`-sheet authorization as a hot-path cost. Loaded frontend modules add overlapping polls/background calls, including attendance state every 30s, attendance main state every 60s, employee manager strips making two parallel calls every 60s, customer feedback every 120s, press status every 120s, general refresh every 180s, Go-Live sweep every 5 minutes, plus focus/visibility refreshes. The every-minute D1 live-sync execution can overlap the web traffic. Worker source exposes public D1 read routes and protected migration/mirror import routes, but no authenticated employee-session edge gateway and no authoritative business-write API; CORS alone is not an authorization boundary. `customer_manager_d1_bridge_v1934.js` contains a direct read-first prototype but is not loaded by `index.html` or `config.js`.
- **Status:** PASS — severe backend latency and request accumulation are independently confirmed; primary causes are the Apps-Script-first/authentication bottleneck plus client polling fan-out, amplified by overlapping D1 sync. Cloudflare is present as a read-first data API, but current source is not yet safe for a full authenticated frontend cutover.
- **Production impact:** NONE. Read-only Apps Script execution inspection and repository analysis only. Version 146, Master/HEALTH, business flags, Fast Auth, triggers, routes, source Sheets, Script Properties, Registry state, `Code.gs`, GitHub Pages, and Cloudflare deployment were not changed.
- **Commit / CI:** Diagnosis ledger commit `a83c61473708bfa61fd2bb7d6a4c67c5531e2a24`; no source CI was required for this read-only/documentation step. No production or Cloudflare deployment was performed.
- **Rollback:** Not applicable to the read-only diagnosis. Revert this documentation entry only if later runtime evidence disproves the measurements.
- **Exact next step:** Define and test the smallest secure Cloudflare edge-gateway increment: explicit authenticated route contract, preserved Apps Script fallback/write authority, request coalescing/visibility-aware polling controls, and no traffic cutover. Verify locally and in CI, ledger the result, then require a separate deploy/cutover gate backed by Worker health, D1 parity/freshness, authentication, fallback, and rollback evidence.
