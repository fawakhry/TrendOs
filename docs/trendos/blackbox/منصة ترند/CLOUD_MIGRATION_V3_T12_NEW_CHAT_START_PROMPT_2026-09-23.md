## LATEST NEW-CHAT PROMPT — Entry 146 owner-directed operational PAUSE / 2026-09-23

> **البرومبت الأحدث للمالك، انقله للشات الجديد. النصوص القديمة تحت تاريخية ولا تُعد توجيهات جديدة.**
>
> «كمّل مشروع TrendOS من **آخر نقطة متوثقة فقط** في الفرع `cloud-migration-v3-t12-order-create-ci-20260919` بمستودع `fawakhry/TrendOs`. أولًا افتح الفرع وتأكد من **HEAD الحالي**، واقرأ `docs/trendos/blackbox/منصة ترند/TRENDOS_T12_CLOUDFLARE_TEST_EXECUTION_JOURNAL_2026-09-21.md` خصوصًا Entries 140–146، و`CLOUD_MIGRATION_V3_T12_CONTINUATION_HANDOFF_2026-09-22.md`. **ما تكررش** أي فحص/خطوة موثّقة، وما تبدأش باقتراح قاعدة TEST أو جداول أو Worker جديد أو إعادة اختبار الأوردر الوهمي. سجّل كل خطوة نجحت أو فشلت أو توقفت في Entry فريدة append-only على نفس الفرع، مع المصدر/الـHEAD قبلها والنتيجة والـcommit وحدود اللي لسه مجهول.
>
> **تقسيم الشغل إلزامي:** ChatGPT Work بيوصل لجوجل شيت **فقط**. Cloudflare هفتحه وأشغّل خطواته **بإيدي** بناءً على توجيه واحد دقيق منك في كل مرة؛ ما تبعتنيش لـWork على Cloudflare وما تدّعيش إنك نفّذت SQL بنفسك. ما تطلبش أسرار أو بيانات عملاء.
>
> **دليل الشغل اللي خلص:** Work أفاد إن آخر صف فيه بيانات، محسوبًا صف العناوين، في الشيت الرئيسي يوم 23/09/2026 الساعة **20:45 القاهرة**: `الأوردرات=705` و`بنود الأوردرات=761` (Entry 141؛ ما تعتبرش الأرقام لقطة حية طول الوقت). قراءة SQL **يدوية، من قاعدة الإنتاج trendos-main**، أظهرت صفوف D1 الفعلية والـcatalog `652` و`708` (Entry 142، وقت قراءة مختلف). اتأكدنا إن جدولي staging الإنتاجيين `sheet_staging_catalog` و`sheet_staging_rows` موجودين (Entry 143)، وإن عدد سجلات كل واحد صفر **وقت الفحص** (Entry 144). أفدتُ إن شاشة Triggers في مشروع Apps Script الأصلي **فاضية** (Entry 145)، وده مش إثبات إن مفيش أي كاتب يدوي/خارجي على D1. ما تطلبش إعادة نفس الصور أو الاستعلامات بدون سبب جديد.
>
> **النطاق التقني الصحيح:** نصلّح مرآة D1 لتابَي `الأوردرات` و`بنود الأوردرات` الحاليين فقط. دوال V2 بتغطي **تابين مش أربعة**. مسار full stage → atomic promote الموجود ممكن يستبدل **كل صفوف التابين** وليس 128 موضعًا فقط، و`d1OrdersLiveSyncTickV2` ممكن يكتب Script Properties baseline قبل التأكد من التطابق ويشغّل rebase تلقائيًا. ما تشغّلوش عمياني؛ مسار R4 القديم محدود 64 إجمالي/5 نمو لكل تاب. نموذج packed-CAS ذي 36 عبارة نجح **محليًا على SQLite فقط**؛ ما جربناهوش على Cloudflare. قاعدة TEST القديمة خصصناها لإنشاء أوردر وهمي، واتجرب إنشاء واحد وإعادة نفس المفتاح مرة، وقفّلنا Worker بعدها؛ ما نعيدش ده ولا ننشئ موارد تجريبية.
>
> **حدود الإنتاج:** Google Sheets/Apps Script ما زال مصدر إنشاء وترقيم الأوردرات الحقيقية. لم تتم استعادة مرآة D1، ولا رجّعنا R5، ولا نقلنا CREATE للسحابة. ممنوع حذف Replay Properties أو إعادة عداد أو Deploy أو استرجاع Trigger تلقائي. قبل أي كتابة D1 إنتاجية لازم حالة مصدر وD1 **حديثة ومتسقة**، معرفة أي كتّاب منافسين، حدود السعة والـScript Properties، خطة التعامل مع فشل/رد مفقود بدون retry أعمى، وفحص تطابق مستقل بعد التنفيذ. اختيار نطاق الكتابة والموافقة عليها منفصل عن الموافقة على الفحص.
>
> **طلبت إيقاف الخطوات التشغيلية في نهاية الشات السابق عشان أنقل البرومبت.** لما أطلب الاستكمال، راجع أولًا هل خطوة قراءة `TRENDOS_R4_RECOVERY_ENABLED` القديمة تخدم فعلًا مسار V2 اللي هنختاره؛ ما تخلّينيش ألف على إعدادات ملهاش صلة. اديني خطوة عملية واحدة تفيد استعادة المرآة باستخدام الموجود، وسجّل نتيجتها فورًا، من غير وعود بموعد تشغيل المنصة قبل التأكد من حالتها الفعلية. لو السؤال عن القدرة على استخدام المنصة حاليًا، ميّز بين الشغل اللي لسه شغال عبر Google وبين الخصائص المعتمدة على D1 المتأخرة.»
>
> **هذا تحديث توثيق/تسليم فقط؛ لا SQL ولا Apps Script ولا Cloudflare live action نُفّذ هنا.**

## CURRENT TECHNICAL CORRECTION — Entry 137 (2026-09-23)

**قبل أي تعليمات قديمة:** راجع Entry 137 في الجورنال؛ كلامنا السابق عن إن V2 بيعمل Full refresh لأربع تابات/الأرشيف كان **غلط**. الكود الموجود في `cloudflare-d1/D1_Orders_Live_Sync_V2.gs` بالدالتين `d1OrdersLiveSyncV2Names_` و`d1OrdersLiveSyncV2CaptureAll_` بيختار **الأوردرات وبنود الأوردرات الحاليين فقط**. مسار V2 الحالي بيجهّز نسخ كاملة للتابين ويرقّيهم معًا، لكن ده مش تحديث 128 موضع صف فقط. **ممنوع تشغّل `d1OrdersLiveSyncTickV2` أو StartV1 مباشرة:** V2 ممكن يعمل Full rebase تلقائي ويكتب Script Properties baseline قبل إثبات تطابق D1، ومسار promote يمسح صفوف النسخة القديمة ويستورد staged من غير CAS على الصور القديمة. لازم إثبات fresh runtime source/D1 والمزامنات المنافسة والسعة/القفل/rollback قبل أي كتابة. ما طلبناش ولا نفذنا كتابة إنتاجية. المالك طلب تحديد الصح وتنفيذه؛ اللي اتنفذ بأمان لحد دلوقتي مراجعة GitHub والتصحيح والتسجيل، لأن صلاحية Apps Script/Cloudflare التشغيلية المباشرة غير متاحة للجلسة ومخاطر الكتابة لسه غير مؤهلة. لا تطلب منه إعادة اختبارات/قاعدة/Worker أو اختيار "تابين ولا أربع" تاني؛ النطاق المقصود التابين الحاليين فقط. لا تشغيل R5 ولا نقل CREATE من Google.
