...
## R4 authenticated Apps Script access and production CAS candidate — 2026-09-20

The owner approved the previously documented production recovery procedure and supplied an authenticated browser session for the original bound Apps Script project `1aGQ5jJ4yYFI5QwMNSM6s1er4LlPbril3kD5nRApScEN-SsNDMXBWm_Eo`. Read-only UI inspection found **zero project triggers**. The latest visible 50 executions contained no Orders low-usage, V2 sync, or operational-enrichment handler and no visible running execution; normal Version 155 web-app traffic remained active. Project settings showed the Orders live-sync V1 flag disabled. No trigger, property, Sheet, deployment or function execution was changed in this inspection. A fresh complete property-quota total was not inferred from the virtualized settings table; the last sanitized audit remains the current documented quota evidence until the read-only audit is rerun.

Prepared an exact production compare-and-swap route candidate `/v1/admin/r4/orders-recovery/apply`, wired only through `production-shadow/index.js`. It is fail-closed unless `TRENDOS_R4_RECOVERY_ENABLED=true`, requires the existing migration-secret authentication, verifies the exact target marker `trendos-main/5c4b92bf-e043-4f6e-bd6d-d514a92cd825`, caps the request at 262144 bytes and 64 candidate upserts, validates exact two-tab shape, then submits all catalog guards, per-row D1 pre-image guards and both catalog advances in one `DB.batch`. It never authorizes trigger restart and returns `automaticRetryAllowed=false`; an ambiguous commit must be reconciled by GET, never retried blindly.

Added the matching one-time Apps Script caller `trendosD1DirectCasBoundedCatchup20260920`. It holds the Script Lock, refuses any tracked sync trigger, captures the authoritative two-tab source, reads D1 directly, constructs only changed/new rows with their current D1 pre-images, rechecks source fingerprint and D1 catalog stability before POST, and does not write Sheets, Script Properties, the baseline or triggers. It uses only the new guarded route, never `/v1/mirror/delta`, `startD1OrdersLowUsageSyncV1()` or `d1OrdersLiveSyncTickV2()`.

Local qualification PASS: targeted planner, guarded SQLite batch/rollback/stale-proposal cases, preview isolation, production route target/auth/default-off checks and Apps Script static safety checks. At this checkpoint the production Worker route is **not deployed**, its temporary enable flag is not enabled, the Apps Script helper is **not yet saved or run**, D1 is unchanged and synchronization remains paused. Cloudflare deployment will be performed manually by the owner; the assistant will perform the authorized Google-side save/run only after the owner confirms the exact route is deployed and enabled.

**Google-side staging completed without execution:** In the authenticated original Apps Script editor, the assistant saved exactly one new Head-only file `R4_D1_CAS_Recovery_20260920.gs`; the function `trendosD1DirectCasBoundedCatchup20260920` is visible in the function picker. A transient empty file created during the UI operation was deleted before final save. The project reports Saved to Drive. The recovery function was not run, no Apps Script deployment was created, and trigger/property/Sheet/baseline state was not mutated.

Added manual GitHub workflow `TrendOS R4 Production Recovery Controlled` so the owner can perform the Cloudflare portion without pasting code in the dashboard. It requires the exact confirmation phrase, validates Worker/D1/entrypoint/UUID and R4 route, reruns the guarded recovery tests, then deploys only `trendos-d1-api`. The `enable` action changes the recovery flag only in the ephemeral runner and expects a non-mutating GET response `405 method-not-allowed`; the `disable` action redeploys the repository default-OFF config and expects `423 recovery-disabled-or-wrong-target`. Neither action calls the recovery POST or mutates D1. The owner must run `enable` manually, then notify the assistant before the single Google execution; after GET-only parity the owner must run `disable`.
