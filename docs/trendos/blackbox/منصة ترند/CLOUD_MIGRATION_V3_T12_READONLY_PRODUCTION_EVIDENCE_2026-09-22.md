...
## Owner-supplied D1 catalog sync times / 2026-09-23

The owner sent a cropped result of the previously requested **read-only** `SELECT sheet_name AS tab_name, synced_at AS last_sync_at FROM sheet_catalog ... ORDER BY sheet_name` in the same Cloudflare D1 console reached from the owner-verified Worker `trendos-d1-api`, binding `DB` → `trendos-main`. The screenshot clearly displays four values (literal database timestamp strings; database's declared timezone/clock have NOT been independently attested):

| Tab | D1 catalog `synced_at` visible in owner screenshot | Catalog row-count from preceding Query 2 |
| --- | --- | ---: |
| أرشيف الأوردرات | `2026-08-29 15:28:53` | 2765 |
| أرشيف بنود الأوردرات | `2026-08-29 15:33:20` | 3984 |
| الأوردرات | `2026-09-20 17:43:19` | 652 |
| بنود الأوردرات | `2026-09-20 17:43:19` | 708 |

**Interpretation:** Against user-facing date 2026-09-23, the D1 catalog's last recorded updates are earlier for **all four** tabs: current Orders/Lines on September 20, archived Orders/Lines on August 29. This corroborates the earlier catalog/source row-count discrepancy and makes it unsafe to use these raw mirrors as a current Cloud CREATE parity source or order-ID allocator high-water. `status='ready'` and equality of `catalog.row_count` to mirrored `sheet_rows` demonstrate only **D1-internal state**, not parity with live Google. `synced_at` is catalog metadata and may not prove every mirror row's last content change or identify the cause of stopped/delayed writes. We have NOT observed the exact failure cause, R5 active runtime flag/trigger, successful recovery, a consistent two-system snapshot, or actual full source-to-mirror row equality; DO NOT assert any of those.

**Operational distinction:** Google Sheets/Apps Script remain sole real production business CREATE and numeric-ID authority; owner previously reported `NEXT_ORDER_NO=4308` and a sequential Sheets observation saw max 4307, but these are historical and non-atomic, and the stale D1 numbers must NOT be used to reset/reseed Google or grant Cloud ID authority. No mirror catch-up, SQL write, R5 restart, trigger creation, Apps Script Head/deploy or TEST/create/replay is authorized or performed by this evidence check. Do not run prepared Query 3 or 4 for a cutover seed until mirror freshness/source lineage is separately qualified; their future read-only diagnostic use would require clearly distinguishing stale baselines from live truth.

**Next safe investigation:** Review the existing production incident/quota/R4 recovery ledger and metadata-only diagnostics without modifying Google/Cloudflare. If live-source health or recurrence needs proof, obtain bounded read-only evidence with owner permission, not a guess from historical repository configs. Owner screenshot bytes, account IDs, DB UUIDs, all customer fields and raw business data are intentionally absent from this GitHub report.
