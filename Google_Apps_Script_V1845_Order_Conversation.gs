 ...
 * نسخة كاملة موحدة V1845: أرقام أوردرات صغيرة بدون حروف + TrendOS + Matbagy Bridge + Pricing Fix:
 * 1) تسجيل الدخول
...
function dashboardMatchesScreen_(screen, department, heatPress) {
  ...
  // V1845: الأقسام في بوابة العميل تُفصل كبنود طباعة/ليزر.
...
function submitCustomerDraft_(e) {
  ...
  upsertOrderSummary_(common);
  const lineResults = [];
  const itemLineMap = {}; // V1845: ربط رقم بند المسودة برقم بند الأوردر النهائي حتى تظهر ملفات الليزر والطباعة في محادثة الموظف.
...
/*********************** V1845 - محادثة الأوردر للموظف والملفات ***********************/
...
function collectOrderPortalFiles_(orderId, lineId) {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (lineId && !lid) continue; // الملفات القديمة قبل V1845 تظهر على مستوى الأوردر كله فقط.
    const record = normalize_(valueAt_(row, colRecord));
