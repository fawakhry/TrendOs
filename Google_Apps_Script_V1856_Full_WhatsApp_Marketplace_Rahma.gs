...
function doGet(e) {
  ...
  try {
    ...
    else if (action === "getReminderNotes") result = getReminderNotes_(e);
    else if (action === "addReminderNote") result = addReminderNote_(e);
    else if (action === "deleteReminderNote") result = deleteReminderNote_(e);
...
function pickupDayCanonical_(value) {
  const raw = normalize_(value);
  if (!raw) return "";
  const key = raw.replace(/[إأآ]/g, "ا").replace(/ة/g, "ه").replace(/ى/g, "ي");
  const map = {
    "الاحد": "الأحد", "احد": "الأحد", "حد": "الأحد",
    "الاثنين": "الاثنين", "الاثنين": "الاثنين", "اثنين": "الاثنين", "اتنين": "الاثنين", "الاتنين": "الاثنين", "الاتنين": "الاثنين",
    "الثلاثاء": "الثلاثاء", "ثلاثاء": "الثلاثاء", "التلات": "الثلاثاء", "تلات": "الثلاثاء", "الثلاثا": "الثلاثاء",
    "الاربعاء": "الأربعاء", "اربعاء": "الأربعاء", "الأربعاء": "الأربعاء", "اربع": "الأربعاء",
    "الخميس": "الخميس", "خميس": "الخميس",
    "الجمعه": "الجمعة", "الجمعة": "الجمعة", "جمعه": "الجمعة", "جمعة": "الجمعة",
    "السبت": "السبت", "سبت": "السبت"
  };
  return map[raw] || map[key] || "";
}

function pickupDaysText_(value) {
  const raw = normalize_(value);
  if (!raw) return "";
  const parts = raw.split(/[،,\/|+\-\n]+| و /g);
  const days = [];
  for (let i = 0; i < parts.length; i++) {
    const d = pickupDayCanonical_(parts[i]);
    if (d && days.indexOf(d) === -1) days.push(d);
  }
  const known = ["الأحد", "الاثنين", "الثلاثاء", "الأربعاء", "الخميس", "الجمعة", "السبت"];
  for (let j = 0; j < known.length; j++) {
    if (raw.indexOf(known[j]) !== -1 && days.indexOf(known[j]) === -1) days.push(known[j]);
  }
  return days.join("، ");
}

function pickupDaysArray_(value) {
  const txt = pickupDaysText_(value);
  return txt ? txt.split("، ").filter(String) : [];
}

function dayNameAr_(date) {
  const names = ["الأحد", "الاثنين", "الثلاثاء", "الأربعاء", "الخميس", "الجمعة", "السبت"];
  const d = new Date(date.getTime ? date.getTime() : date);
  return names[d.getDay()];
}

function customerHasPickupTomorrow_(pickupDays, today) {
  const days = pickupDaysArray_(pickupDays);
  if (!days.length) return false;
  const tomorrow = addDays_(today || startOfToday_(), 1);
  return days.indexOf(dayNameAr_(tomorrow)) !== -1;
}

function nextCustomerPickupDate_(fromDate, pickupDays) {
  const days = pickupDaysArray_(pickupDays);
  if (!days.length) return null;
  const base = new Date(fromDate.getTime ? fromDate.getTime() : fromDate);
  base.setHours(0, 0, 0, 0);
  for (let i = 1; i <= 14; i++) {
    const d = addDays_(base, i);
    if (days.indexOf(dayNameAr_(d)) !== -1) return d;
  }
  return null;
}
...
function searchCustomers_(e) {
  ...
  const colPickupDays = firstCol_(h, ["أيام استلام العميل", "أيام التسليم", "مواعيد استلام العميل", "Pickup Days"], 0);
  const colActive = firstCol_(h, ["مفعل؟", "مفعل", "Active"], 0);
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    const pickupDays = colPickupDays ? pickupDaysText_(row[colPickupDays - 1]) : "";

    const blob = searchKey_([name, manager, phone, extra, type, pickupDays].join(" "));
    if (blob.indexOf(q) !== -1) {
      const key = name + "|" + phone;
      if (!seen[key]) {
        seen[key] = true;
        out.push({ name: name, manager: manager, phone: phone || extra, extraPhone: extra, type: type, pickupDays: pickupDays, deliveryDays: pickupDays });
      }
    }
...
/*********************** كنترول الأوردرات المفتوحة قبل التسجيل ***********************/

function isOpenOrderForDuplicateControl_(status) {
  const s = normalize_(status) || "طلب جديد";
  // جاهز للاستلام يعتبر لسه مفتوح لأن العميل لم يستلم بعد.
  return ["تم التسليم", "ملغى", "مكرر"].indexOf(s) === -1;
}

function findOpenCustomerOrders_(customerName, customerPhone, limit) {
  const nameKey = searchKey_(customerName);
  const phoneKey = cleanPhone_(customerPhone);
  const outLimit = Number(limit || 10) || 10;
  ...
  const colExpected = firstCol_(h, ["الوقت المتوقع", "تاريخ التسليم المتوقع", "Expected Delivery"], 0);

  const grouped = {};
  for (let i = data.length - 1; i >= 1; i--) {
    const row = data[i];
    const status = normalize_(valueAt_(row, colStatus));
    if (!isOpenOrderForDuplicateControl_(status)) continue;

    const rowCustomer = normalize_(valueAt_(row, colCustomer));
    const rowNameKey = searchKey_(rowCustomer);
    const rowPhone = cleanPhone_(valueAt_(row, colPhone));
    const matchedByPhone = !!(phoneKey && rowPhone && phoneKey === rowPhone);
    const matchedByName = !!(nameKey && rowNameKey && nameKey === rowNameKey);
    if (!matchedByPhone && !matchedByName) continue;

    const orderId = normalize_(valueAt_(row, colOrderId)) || normalize_(valueAt_(row, colLineId)) || "-";
    if (!grouped[orderId]) {
      grouped[orderId] = {
        orderId: orderId,
        ...
        expectedDeliveryText: dateText_(valueAt_(row, colExpected)) || normalize_(valueAt_(row, colExpected)),
        updatedAt: dateText_(valueAt_(row, colUpdated)) || normalize_(valueAt_(row, colUpdated)),
        matchedBy: matchedByPhone ? "phone" : "name",
        linesCount: 0
      };
    }
  ...
  return Object.keys(grouped).map(function(k) { return grouped[k]; }).slice(0, outLimit);
}

function checkOpenCustomerOrders_(e) {
  const auth = authorize_(e.parameter.username, e.parameter.token);
  if (!auth.ok) return { success: false, message: auth.message };
  if (!canCreateOrder_(auth.user)) return { success: false, message: "ليس لديك صلاحية فحص أوردرات العملاء." };

  const customerName = normalize_(e.parameter.customerName);
  const customerPhoneParam = cleanPhone_(e.parameter.customerPhone);
  const customerInfo = customerName ? findCustomerInfoByName_(customerName) : {};
  const customerPhone = customerPhoneParam || customerInfo.phone || customerInfo.extraPhone || "";
  const openOrders = findOpenCustomerOrders_(customerName, customerPhone, 10);
  ...
  return {
    ...
    message: openOrders.length ? "العميل له أوردر مفتوح قبل كده." : "لا توجد أوردرات مفتوحة لهذا العميل."
  };
}
...
function buildCustomerPhoneMap_() {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    const pickupDays = c.pickupDays ? pickupDaysText_(row[c.pickupDays - 1]) : "";
...
function findCustomerInfoByName_(customerName) {
  const nameKey = searchKey_(customerName);
  if (!nameKey) return { name: "", phone: "", extraPhone: "", manager: "", type: "", pickupDays: "", deliveryDays: "" };
  ...
  return { name: normalize_(customerName), phone: "", extraPhone: "", manager: "", type: "", pickupDays: "", deliveryDays: "" };
}
...
function createCustomer_(e) {
  ...
  ensureHeaderIfAnyMissing_(sheet, ["اسم الشات / المكتب", "اسم المسؤول", "رقم العميل الأساسي", "رقم إضافي", "نوع العميل", "أيام استلام العميل", "أيام التسليم", "مفعل؟", "ملاحظات", "تاريخ الإضافة", "آخر تحديث"]);
...
function getRows_(e) {
  ...
  ensureHeaderIfAnyMissing_(lines, ["أيام استلام العميل"]);
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    rows.push({
      ...
      customerPickupDays: customerPickupDays,
      pickupDays: customerPickupDays,
      deliveryDays: customerPickupDays,
...
function createManualOrder_(e) {
  ...
  const customerPickupDays = pickupDaysText_(customerInfo.pickupDays || customerInfo.deliveryDays || e.parameter.customerPickupDays || e.parameter.pickupDays || "");
  ...
  const notes = normalize_(e.parameter.notes);
  const duplicateOverride = normalize_(e.parameter.duplicateOverride) === "نعم" || normalize_(e.parameter.duplicateOverride) === "1" || normalize_(e.parameter.duplicateOverride).toLowerCase() === "true";
  ...
  const openOrdersBeforeCreate = findOpenCustomerOrders_(customerName, customerPhone, 10);
  if (openOrdersBeforeCreate.length && !duplicateOverride) {
    return {
      success: false,
      duplicateWarning: true,
      openOrders: openOrdersBeforeCreate,
      message: "العميل له أوردر مفتوح قبل كده. راجع الأوردر القديم أو أكّد أنه أوردر جديد."
    };
  }
  ...
  const now = new Date();
  const customerNextPickupDate = customerPickupDays ? nextCustomerPickupDate_(now, customerPickupDays) : null;
  const expectedDeliveryAt = flyPrint ? new Date(now) : (customerNextPickupDate || expectedDeliveryDate_(now));
  const expectedDeliveryText = flyPrint ? (formatDateAr_(expectedDeliveryAt) + " - نفس اليوم") : (formatDateAr_(expectedDeliveryAt) + (customerPickupDays ? " - أيام استلام العميل: " + customerPickupDays : ""));
  ...
  return {
    ...
    expectedDeliveryAt: expectedDeliveryAt,
    expectedDeliveryText: expectedDeliveryText,
    duplicateOverride: duplicateOverride ? "نعم" : "لا",
    duplicateOpenOrdersCount: openOrdersBeforeCreate.length,
    message: duplicateOverride && openOrdersBeforeCreate.length ? "تم إضافة الأوردر كطلب جديد رغم وجود أوردر مفتوح." : "تم إضافة الأوردر في الشيتين."
  };
}
...
function createCustomer_(e) {
  ...
  ensureHeaderIfAnyMissing_(sheet, ["اسم الشات / المكتب", "اسم المسؤول", "رقم العميل الأساسي", "رقم إضافي", "نوع العميل", "أيام استلام العميل", "أيام التسليم", "مفعل؟", "ملاحظات", "تاريخ الإضافة", "آخر تحديث", "مديونية", "ملاحظات المديونية", "آخر تحديث مديونية", "كود فرع مطبعجي", "اسم فرع مطبعجي", "آخر تحديث فرع العميل"]);
...
function upsertOrderSummary_(o) {
  ...
  const values = {
    ...
    "تاريخ التسليم المتوقع": o.expectedDeliveryAt || expectedDeliveryDate_(o.now),
    "الوقت المتوقع": o.expectedDeliveryText || expectedDeliveryText_(o.now),
    "أيام استلام العميل": o.customerPickupDays || "",
    "مكبس حراري": o.heatPress ? "نعم" : "لا",
...
function createManualOrder_(e) {
  ...
  return {
    ...
    duplicateOverride: duplicateOverride ? "نعم" : "لا",
    duplicateOpenOrdersCount: openOrdersBeforeCreate.length,
    message: duplicateOverride && openOrdersBeforeCreate.length ? "تم إضافة الأوردر كطلب جديد رغم وجود أوردر مفتوح." : (debtAmount > 0 ? "تم إضافة الأوردر مع تنبيه مديونية العميل." : "تم إضافة الأوردر في الشيتين.")
  };
}
...
function getRows_(e) {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    const customerPickupDays = pickupDaysText_(valueAt_(row, colPickupDays) || customerLookup.pickupDays || customerLookup.deliveryDays || "");
...
function getDashboard_(e) {
  ...
  const tomorrowPickupOrderSet = {}, tomorrowNotClosedOrderSet = {};
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    const customerName = normalize_(valueAt_(row, colCustomer));
    const customerLookup = customerMap[searchKey_(customerName)] || {};
    const customerPickupDays = pickupDaysText_(customerLookup.pickupDays || customerLookup.deliveryDays || "");
    const dept = normalize_(valueAt_(row, colDept)) || "غير محدد";
    const press = isHeatPressFlag_(valueAt_(row, colPress));
    const qty = Number(valueAt_(row, colQty)) || 1;
    const expectedRaw = valueAt_(row, colExpectedAt) || valueAt_(row, colExpectedText);
    ...
    const isTomorrowPickup = (customerHasPickupTomorrow_(customerPickupDays, today) || isSameDay_(expected, tomorrow)) && ["تم التسليم", "ملغى", "مكرر"].indexOf(status) === -1;
    if (isTomorrowPickup) {
      dashboard.tomorrowPickupLines++;
      dashboard.tomorrowPickupSheets += qty;
      if (orderId) tomorrowPickupOrderSet[orderId] = true;
      if (!(isReadyStatus_(status) || status === "تم التسليم" || status === "ملغى" || status === "مكرر")) {
        dashboard.tomorrowNotClosedLines++;
        dashboard.tomorrowNotClosedSheets += qty;
        if (orderId) tomorrowNotClosedOrderSet[orderId] = true;
      }
    }
  ...
  dashboard.todayWorkOrders = Object.keys(todayWorkOrderSet).length;
  dashboard.todayOrders = dashboard.todayWorkOrders;
  dashboard.tomorrowPickupOrders = Object.keys(tomorrowPickupOrderSet).length;
  dashboard.tomorrowNotClosedOrders = Object.keys(tomorrowNotClosedOrderSet).length;
  dashboard.activeOrders = Object.keys(activeOrderSet).length;
  dashboard.deliveredTodayOrders = Object.keys(deliveredTodayOrderSet).length;
  dashboard.readyOrders = Object.keys(readyOrderSet).length;
  dashboard.overdueOrders = Object.keys(overdueOrderSet).length;
...
function emptyDashboard_(screen) {
  const nameMap = { service: "خدمة العملاء", print: "الطباعة", laser: "الليزر", press: "المكبس" };
  return {
    screen: screen || "service", departmentName: nameMap[screen] || "خدمة العملاء", todayOrders: 0, todayWorkOrders: 0, todayWorkLines: 0, todayWorkSheets: 0, todayWorkDoneLines: 0, tomorrowPickupOrders: 0, tomorrowPickupLines: 0, tomorrowPickupSheets: 0, tomorrowNotClosedOrders: 0, tomorrowNotClosedLines: 0, tomorrowNotClosedSheets: 0, activeOrders: 0, activeLines: 0, activeSheets: 0, urgent: 0, normal: 0, delayedPriority: 0, overdue: 0, overdueOrders: 0, problems: 0, readyForPickup: 0, readyOrders: 0, delivered: 0, deliveredToday: 0, deliveredTodayOrders: 0, duplicate: 0, heatPress: 0, debtOrders: 0, completionPercent: 0, timeScore: 100, performanceScore: 0, byDepartment: { "طباعة": 0, "ليزر": 0, "مكبس": 0 }
  };
}
...
/*********************** V1856 Patch 14 - بورد التذكير المشترك ***********************/

function reminderBoardSheet_() {
  return mbEnsureSheet_("بورد الملاحظات", [
    "رقم النوت",
    "النوت",
    "بواسطة",
    "وقت الإضافة",
...
function reminderBoardRows_() {
  const sheet = reminderBoardSheet_();
  const data = sheet.getDataRange().getValues();
  const h = headersMap_(sheet);
  const colId = firstCol_(h, ["رقم النوت", "ID"], 1);
  const colNote = firstCol_(h, ["النوت", "الملاحظة", "Note"], 2);
  const colBy = firstCol_(h, ["بواسطة", "Created By"], 3);
  const colAt = firstCol_(h, ["وقت الإضافة", "Created At"], 4);
  const colDeleted = firstCol_(h, ["محذوف؟", "Deleted"], 5);
  const notes = [];
  for (let i = 1; i < data.length; i++) {
    const row = data[i];
    const deleted = normalize_(valueAt_(row, colDeleted));
    if (deleted === "نعم" || deleted.toLowerCase() === "true" || deleted === "1") continue;
    const noteText = normalize_(valueAt_(row, colNote));
    if (!noteText) continue;
    const at = valueAt_(row, colAt);
    notes.push({
      id: normalize_(valueAt_(row, colId)),
      text: noteText,
      by: normalize_(valueAt_(row, colBy)) || "-",
      at: dateTimeText_(at),
      rowNumber: i + 1
  ...
  notes.sort(function(a, b) { return Number(b.rowNumber || 0) - Number(a.rowNumber || 0); });
  return notes;
}
...
function getReminderNotes_(e) {
  const auth = authorize_(e.parameter.username, e.parameter.token);
  if (!auth.ok) return { success: false, message: auth.message };
  const limit = Math.max(1, Math.min(200, Number(e.parameter.limit || 50)));
  return { success: true, notes: reminderBoardRows_().slice(0, limit) };
}

function addReminderNote_(e) {
  const auth = authorize_(e.parameter.username, e.parameter.token);
  if (!auth.ok) return { success: false, message: auth.message };
  const note = normalize_(e.parameter.note || e.parameter.text || "");
  if (!note) return { success: false, message: "اكتب النوت أولاً." };
  if (note.length > 1000) return { success: false, message: "النوت طويلة جداً. اختصرها تحت 1000 حرف." };
  const sheet = reminderBoardSheet_();
  const id = "N-" + Utilities.getUuid().slice(0, 8);
  sheet.appendRow([id, note, auth.user.username || normalize_(e.parameter.username), new Date(), "", "", ""]);
  SpreadsheetApp.flush();
  return { success: true, message: "تمت إضافة النوت.", notes: reminderBoardRows_().slice(0, 50) };
}

function deleteReminderNote_(e) {
  const auth = authorize_(e.parameter.username, e.parameter.token);
  if (!auth.ok) return { success: false, message: auth.message };
  const noteId = normalize_(e.parameter.noteId || e.parameter.id || "");
  if (!noteId) return { success: false, message: "رقم النوت غير موجود." };
  const sheet = reminderBoardSheet_();
  const data = sheet.getDataRange().getValues();
  const h = headersMap_(sheet);
  const colId = firstCol_(h, ["رقم النوت", "ID"], 1);
  const colDeleted = firstCol_(h, ["محذوف؟", "Deleted"], 5);
  const colDeletedAt = firstCol_(h, ["وقت الحذف", "Deleted At"], 6);
  const colDeletedBy = firstCol_(h, ["حذف بواسطة", "Deleted By"], 7);
  for (let i = 1; i < data.length; i++) {
    if (normalize_(valueAt_(data[i], colId)) === noteId) {
      safeSet_(sheet, i + 1, colDeleted, "نعم");
      safeSet_(sheet, i + 1, colDeletedAt, new Date());
      safeSet_(sheet, i + 1, colDeletedBy, auth.user.username || normalize_(e.parameter.username));
      SpreadsheetApp.flush();
      return { success: true, message: "تم مسح النوت.", notes: reminderBoardRows_().slice(0, 50) };
    }
  }
  return { success: false, message: "النوت غير موجودة أو تم مسحها." };
}
